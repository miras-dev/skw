...
- **Check for updates** is the gentle, repeatable option, offered once a clan already has a roster. It runs the exact same ranking underneath, but instead of overwriting anything, it just shows you a list of *candidates worth adding* — players who aren't already tracked for this clan. Nothing is written until you personally click a slot button for a specific candidate. Two kinds of players show up here:
  - **Any untracked player currently in a Legend league** — shown regardless of how their recent form/score ranked them. The reasoning baked into the code: reaching Legend league is a much harder-won signal than a few days of recent attacks, so a Legend player having an average week shouldn't get silently ignored just because the ranking algorithm's score-based cut would have excluded them.
  - **Any other untracked player the ranking model would actually put on the roster** (i.e., they'd make the model's top-15/30 pick).

  Legend candidates are flagged with a gold "Legend" pill and get a genuinely open choice of Main/Sub/Not-selected. Non-Legend candidates get an explicit note that they default to Not-selected unless you deliberately place them somewhere else, and their recommended button gets a little checkmark.
